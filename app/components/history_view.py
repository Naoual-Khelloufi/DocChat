...
def render():
    ...
    with col_chat:
        ...
        for h in messages:
            # ── entête : heure + poubelle ────────────────────────────
            head_col, trash_col = st.columns([10, 1])
            head_col.markdown(f"**🕑 {h.timestamp.strftime('%H:%M:%S')}**")
            if trash_col.button("🗑️", key=f"del-msg-{h.id}", help="Supprimer"):
                crud.delete_message(db, h.id, uid)
                
            # 📄  Afficher PDF associé
            head_col, view_col, trash_col = st.columns([9, 1, 1])
            if h.document_id and view_col.button("📄", key=f"view-{h.id}", help="Voir le PDF"):
